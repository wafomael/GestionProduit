...
class ConsoleAffichage(Affichage):
    ...
    def afficherProduit(self,produits):
        entete = f"{'ID':<10}{'Nom':<20}{'Quantité':<10}{'Prix Unitaire ($)':<20}{'Périssable':<15}{'Date Péremption':<20}"
        print(entete)
        print("-" * len(entete))
        ...
        for produit in produits:
            if produit == None:
                ...
                print(f"{idProduit:<10}{nom:<20}{quantite:<10}{prixUnitaire:<20}{isperisable:<15}{str(datePremption):<20}")
    ...
    def afficherVente(self,ventes):
        ...
        for vente in ventes:
            print(
                f"{vente.getIdVente():<10} {vente.getIdProduit():<12} {vente.getQuantite():<10} {str(vente.getDateResemption()):<15} {vente.getPrice():<10.2f}")
